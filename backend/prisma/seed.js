...
async function main() {
  try {
    ...
    const servicesDowntown = await Promise.all(
      serviceTemplates.map((service) =>
        prisma.service.create({
          data: { branchId: branchDowntown.id, ...service },
        }),
      ),
    );
    const servicesUptown = await Promise.all(
      serviceTemplates.map((service) =>
        prisma.service.create({
          data: { branchId: branchUptown.id, ...service },
        }),
      ),
    );
    ...
    const booking1 = await prisma.booking.create({
      data: {
        ...
        bookingServices: {
          create: [
            {
              serviceId: servicesDowntown[0].id,
              quantity: 1,
              unitPrice: 150000,
              subtotal: 150000,
            },
    ...
    console.log(
      '🔑 Login Credentials: Admin: username "admin" / email "admin@barbershop.com", password "password123"',
    );
  } catch (error) {
