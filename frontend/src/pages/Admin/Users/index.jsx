...
import { deleteUser, getUsers, updateUser, createUser } from "../../../services/adminUserApi";
import "./Users.css";
...
function Users() {
    ...
    const loadUsers = async (nextPage = page, nextQuery = query) => {
        ...
        const response = await getUsers({ page: nextPage, limit: PAGE_SIZE, q: nextQuery });
        ...
        if (response.success) {
            ...
            setUsers([]);
            setPagination({ page: nextPage, limit: PAGE_SIZE, total: 0, totalPages: 0 });
        ...
        <section className="page-panel users-page">
            <div className="page-toolbar users-toolbar">
                ...
                <div className="page-actions users-actions">
                    <form className="users-search" onSubmit={handleSearchSubmit}>
                        ...
                        />
                        <button type="submit" className="btn btn-primary" disabled={loading || isSaving}>
                            Search
                        </button>
                    </form>
            ...
            {loading ? (
                ...
                <div className="dashboard-state dashboard-state-error">
                    <p>{error}</p>
                    <button type="button" className="btn btn-primary" onClick={() => loadUsers(page, query)}>
                        Retry
                    </button>
                </div>
            ) : users.length === 0 ? (
