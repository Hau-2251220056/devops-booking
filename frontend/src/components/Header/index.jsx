    ...
    <header className="site-header">
      <nav className="header-inner">
        <div className="logo-wrap">
          <Link to="/" className="logo">
            <div className="logo-icon"><FaCut /></div>
        ...
        <div className="header-actions">
          ...
          <button className="mobile-toggle" onClick={toggleMenu}>{isMenuOpen ? <FaTimes /> : <FaBars />}</button>
        </div>
      </nav>
      ...
      {isMenuOpen && (
        <div className="mobile-menu">
          {menuItems.map((item) => (
            <a key={item.label} href={item.href} className="mobile-item" onClick={() => setIsMenuOpen(false)}>{item.label}</a>
          ))}
